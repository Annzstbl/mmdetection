...
@MODELS.register_module()
class HyperspecImgDataPreprocessor(BaseDataPreprocessor):
    ...
    def __init__(self,
        ...
        if mean is not None:
            assert len(mean) == len(std), f'`mean` and `std` should have the same length, but got mean {len(mean)} and std {len(std)}'
        ...
        self._channel_conversion = False # 无rgb转变
        ...
        if is_seq_of(_batch_inputs, torch.Tensor):
            ...
            for _batch_input in _batch_inputs:
                ...
                if self._enable_normalize:
                    assert _batch_input.dim() == 3 and _batch_input.shape[0] == self.mean.shape[0], (
                        'The shape of mean and input tensor does not match,'
                        ' got the input tensor with shape'
                        f'{_batch_input.shape} and  got the mean with shape {self.mean.shape}')
...
@MODELS.register_module()
class DetHyperspecDataPreprocessor(HyperspecImgDataPreprocessor):
    """高光谱处理
